...
export class ReviewService extends BaseService {
  ...
  async submitReview(reviewerId: string, dto: SubmitReviewDto) {
    ...
    if (dto.refType === "order") {
      ...
      if (order.status !== ORDER_STATUS.COMPLETED) {
        throw new ForbiddenError("You can only review after the order is completed");
      }

      // Only the buyer reviews the seller
      if (order.buyerId.toString() !== reviewerId) {
        throw new ForbiddenError("Only the buyer can leave a review for a service order");
      }
      ...
      if (errand.status !== ERRAND_STATUS.CONFIRMED) {
        throw new ForbiddenError("You can only review after the errand is confirmed");
      }

      if (errand.posterId.toString() !== reviewerId) {
        throw new ForbiddenError("Only the errand poster can leave a review for the runner");
      }
  ...
  async getUserReviews(revieweeId: string, opts: PaginationOptions = {}) {
    return paginate(
      Review,
      { revieweeId: new mongoose.Types.ObjectId(revieweeId) },
  ...
  private async _updateRatingStats(userId: string): Promise<void> {
    const stats = await Review.aggregate<{ avgRating: number; total: number }>([
